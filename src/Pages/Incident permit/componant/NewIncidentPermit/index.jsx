...
const New_Incident_Permit = () => {
  ...
  const [tabdisable, setTabDiseble] = useState(true);
  ...
  const validateForm = (id) => {
    ...
    if (!incidentpermit?.summary) {
      setErrorValue(true);
      formIsValid = false;
      errors['summary'] = t('please_enter_summary');
    }

    if (!incidentpermit?.businessUnitId) {
      setErrorValue(true);
      formIsValid = false;
      errors['BuisnessUnit'] = t('please_select_buisness_unit');
    }
    if (!incidentpermit?.processId) {
      setErrorValue(true);
      formIsValid = false;
      errors['ptype'] = t('please_select_incident_type');
    }

    if (!incidentpermit?.severityid) {
      setErrorValue(true);
      formIsValid = false;
      errors['saverity'] = t('please_select_severity');
    }
    if (!incidentpermit?.date) {
      setErrorValue(true);
      formIsValid = false;
      errors['Incident Date'] = t('please_select_incident_date');
    }
    if (!incidentpermit?.location) {
      setErrorValue(true);
      formIsValid = false;
      errors['zone'] = t('please_select_location');
    }
    if (!incidentpermit?.date) {
      setErrorValue(true);
      formIsValid = false;
      errors['date'] = t('please_select_date');
    }
    if (!incidentpermit?.department) {
      setErrorValue(true);
      formIsValid = false;
      errors['department'] = t('please_enter_department');
    }
    if (!incidentpermit?.shiftid) {
      setErrorValue(true);
      formIsValid = false;
      errors['shift'] = t('please_select_shift');
    }
    if (!incidentpermit?.involved) {
      setErrorValue(true);
      formIsValid = false;
      errors['involved'] = t('please_select_incident_involved');
    }
    if (!incidentpermit?.potentialid) {
      setErrorValue(true);
      formIsValid = false;
      errors['potential'] = t('please_select_potential_severity');
    }
    if (!incidentpermit?.siteid) {
      setErrorValue(true);
      formIsValid = false;
      errors['site'] = t('please_select_site');
    }
    if (!incidentpermit.riskmatrix) {
      setErrorValue(true);
      formIsValid = false;
      errors['riskmatrix'] = t('please_select_risk_matrix');
    }
    if (!incidentpermit?.reportBy && !ValueAuto) {
      setErrorValue(true);
      formIsValid = false;
      errors['reportBy'] = t('please_select_report_by');
    }
    if(!isNew){
  ...
  const handleUpdate = async () => {
    ...
    try {
      if (checkListValidation()) {
       ...
       // incidentpermit['Signature'] = incidentpermitsignature;
        incidentpermit['date'] = moment(incidentpermit?.date).format();
        incidentpermit['reportBy'] = incidentpermitapplicant;
        setIsUpdating(true);
  ...
  const getData = async (id) => {
    ...
    else if (res.data.responseObject.statusId === 24) {
      setStepperValue(4);
    } 
    else{
      const newindex = stepperData.findIndex((val, index) => {
        return res.data.responseObject.statusDesc === val.statusDesc;
      });
  ...
  const getApplicable = async () => {
    try {
      const res = await getWorkFlowListApplicable();
  ...
  const handlePrint = () => {
    ...
    const newWindow = window.open(
     ...
     processValue === true ? `#/print/${incidentpermitID}` : '#/print-checklist',
      '_blank',
      'noopener,noreferrer',
    );
    ...
    const buttons = [
      {
        ...
        hide: showChecklistsForm || isNew || selected == 7 || stepperName === 'Rejected' || stepperName === 'Close'|| stepperName === 'Cancelled',
        show: !isNew,
        disabled: isUpdating,
      },
    ];
    ...
    <>
      {isNew ? (
        <Row className="mx-1  p-0 shadow align-items-center row m-1 rounded m-0 mainOuter mt-0">
          ...
          <Col xs={12} md={6} className="text-right button-group mb-2 mt-1 ">
          ...
          <Row>
            {isNew ? (
              <Splitter className="tree-space" panes={panes} onChange={onChange}>
                <div className="pane-content sidespace ">
                  {isNew && (
                ...
                <div className="pane-content">
                  <TabContext value={selected}>
                    <TabList onChange={handlePageClick} className="border-bottom">
                      ...
                      {!isNew && checkTabName('Action') && (
                        <Tab label={t('action')} value={2} className="font-weight-bold" />
                      )}

                      {!isNew && checkTabName('INVESTIGATION') && (
                        <Tab
                ...
                <TabContext value={selected}>
                  <TabList onChange={handlePageClick} className="border-bottom">
                    <Tab
                      ...
                      className="font-weight-bold"
                    />
                    {!isNew && checkTabName('INVESTIGATION') && tabdisable === false && (
                      <Tab
                    ...
                    {!isNew && checkTabName('Action') && tabdisable === false && (
                      <Tab
                        label={t('action')}
                        value={2}
                        disabled={savedata}
                    ...
                    {/* {!isNew && dynaminButton[0]?.displayName != 'Submit for investigate'&&( */}
                    {!isNew && dynaminButton[0]?.displayName != 'submit for investigation' &&( 
                      <Tab
                        label={t('audit_trail')}
                        value={5}
                        disabled={savedata}
