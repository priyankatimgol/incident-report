...
var textField = function (options, defaultParameters) {
  ...
  resource.html =
    ...
    (canBeHidden ? 'data-show="isHidden"' : '') +
    '>' +
    // '<select id="camunda-' + escapeHTML(resource.id) + '" type="text" name="' + escapeHTML(options.modelProperty) + '></select>' +
    '<input class="k-input k-input-md k-rounded-md k-input-solid" id="camunda-' + escapeHTML(resource.id) + '" type="text" name="' + escapeHTML(options.modelProperty) + '" ' +
    (canBeDisabled ? 'data-disable="isDisabled"' : '') +
    (canBeHidden ? 'data-show="isHidden"' : '') +
    ' />' +
