...
var DELETE_ROW_BUTTON_SNIPPET =
  '<button class="clear" data-action="deleteElement">' + '<span>X</span>' + '</button>';

function createInputRowTemplate(properties, canRemove, options) {
  var template = TABLE_ROW_DIV_SNIPPET;
  template += createInputTemplate(properties, canRemove, options);
...
function createInputTemplate(properties, canRemove, options) {
  var columns = properties.length;
  var template = '';
  forEach(properties, function (prop) {
    if (prop === 'name') {
      template +=
        '<select style="border-color: black;margin-left:2px !important"  class="k-dropdownlist k-picker  k-picker-md k-rounded-md k-picker-solid" id="camunda-' +
      ...
      template +=
        Object.values(JSON.parse(localStorage.getItem('setCloumnList')))
          .map((e) => {
      ...
      template +=
        '<input style="border-color: black;" type="text" readonly="readonly" class="k-input k-input-md k-rounded-md k-input-solid bpp-table-row-columns-' +
        columns +
        ' ' +
        (canRemove ? 'bpp-table-row-removable' : '') +
        '" ' +
        'id="camunda-table-row-cell-input-value disableInput" ' +
        'disabled="true"' +
        'name="' +
        escapeHTML(prop) +
        '" />';
...
function createLabelTemplate(labels) {
  ...
  forEach(labels, function (label) {
    template +=
      '<label class="bpp-table-row-columns-' + columns + '">' + escapeHTML(label) + '</label>';
  });
  return template;
}
...
module.exports = function (options) {
  ...
  var factory = {
    ...
    createListEntryTemplate: function (value, index, selectBox) {
      return createInputRowTemplate(modelProperties, canRemove, options);
    },

    addElement: function (element, node, event, scopeNode) {
      var template = domify(createInputRowTemplate(modelProperties, canRemove, options));
