...
var getRole = localStorage.getItem('setRole');
...
function createInputTemplate(properties, canRemove, options) {
  ...
  forEach(properties, function (prop) {
    if (prop === 'name') {
      template +=
        '<input type="text" readonly="readonly" style="display:none" class=" k-input k-input-md k-rounded-md k-input-solid bpp-table-row-columns-' +
        columns +
        ' ' +
      ...
      template +=
        JSON.parse(getRole)
          ?.map((e) => {
            return `<option value=${e.roleId}>${e.roleDesc}</option>`;
          })
          .join() + '</select>';
    }
  });
